...
/**
 * Created by Bantu Makers on 11/12/2017.
 */
...
public class ArtistTrackAdapter extends RecyclerView.Adapter<ArtistTrackAdapter.ViewHolder> {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder{
        ...
        public ViewHolder(View itemView){
            super(itemView);
            //Criamos os itens do nosso layout
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        holder.mArtistName.setText(trackList.getArtistTrack().get(position).getArtist().getName());
        holder.mTrackName.setText(trackList.getArtistTrack().get(position).getaName());
        holder.mTrackCover.setImageResource(trackList.getArtistTrack().get(position).getTrackCover());
        if (trackList.getArtistTrack().get(position).getArtist().isVerified()){
    ...
    public int getItemCount() {
        return trackList.getArtistTrack().size();
    }
}
